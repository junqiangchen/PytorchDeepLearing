...
class SMT3d(nn.Module):
    """
    SMT3d implement
    """
    # the number of convolutions in each layer corresponds
    # to what is in the actual prototxt, not the intent
    def __init__(self, image_channel, numclass, img_size=96):
        super(SMT3d, self).__init__()
        self.inputChans = image_channel
        self.outChans = numclass
        self.model = SMT(img_size=img_size, in_chans=self.inputChans, num_classes=self.outChans,
                         embed_dims=[30 * 2, 60 * 2, 120 * 2, 240 * 2], ca_num_heads=[3, 3, 3, -1],
                         sa_num_heads=[-1, -1, 8, 16], mlp_ratios=[2, 2, 2, 2], qkv_bias=True, depths=[2, 2, 4, 2],
...
if __name__ == "__main__":
    net = SMT3d(1, 1, 128)
    in1 = torch.rand((1, 1, 128, 128, 128))
    out_logit, out = net(in1)
